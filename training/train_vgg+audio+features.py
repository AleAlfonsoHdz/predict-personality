...
# feature vector (emotions histogram (35 values) + gender (1 value) + age (1 value)) = 37 values
feature_vector = Input(shape = (37,), name='feature_vector')

dense32_f = Dense(32, activation='relu', name='fc_feature1')(feature_vector)
dense10_f = Dense(10, activation='relu', name='fc_feature2')(dense32_f)


#---------------------------------

models_fusion = concatenate(([vgg_dense32, audio_dense32, dense10_f]), axis=1, name='vgg_audio_features_fusion')
...
vgg_audio_features_model = Model([vgg_model.input, audio_model.input, feature_vector], output)
...
history = vgg_audio_features_model.fit_generator( generator = train_generator, validation_data = validation_generator, epochs = 70, verbose = 1, callbacks = callbacks_list )
...
plt.title('vgg+audio+features_model MSE')
plt.ylabel('MSE')
plt.xlabel('epoch')
plt.legend(['train', 'validation'], loc='upper right')
plt.savefig('plot_vgg+audio+features_model_MSE.png')

# summarize history for mae
figure()
plt.plot(history.history['mean_absolute_error'])
plt.plot(history.history['val_mean_absolute_error'])
plt.title('vgg+audio+features_model MAE')
plt.ylabel('MAE')
plt.xlabel('epoch')
plt.legend(['train', 'validation'], loc='upper right')
plt.savefig('plot_vgg+audio+features_model_MAE.png')

# summarize history for loss
figure()
plt.plot(history.history['loss'])
plt.plot(history.history['val_loss'])
plt.title('vgg+audio+features_model loss')
plt.ylabel('loss')
plt.xlabel('epoch')
plt.legend(['train', 'validation'], loc='upper right')
plt.savefig('plot_vgg+audio+features_model_loss.png')

with open('vgg_audio_features_model_history.pkl', 'wb') as f:
